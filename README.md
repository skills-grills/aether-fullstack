...
* **Health Check** - just as a check to ensure that the API is up and running.
...
* **Create Report** - this route initiates the build process, running the seperate AI async calls.
...
* **Get Report** - gets status update OR completed result.
...
From the above containerised deployment, an AI Report Generator service is generated, and example `curl` REST actions provided. The resulting API can then be actioned upon with `requests` or similar library in Rust/Typescript/Go, for further result parsing and formatting.

> *"Pull Request review initiated, moving on to next JIRA ticket..." -JDB*
